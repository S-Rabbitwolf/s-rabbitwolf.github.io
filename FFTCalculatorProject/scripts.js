...
// Writing additional segments whenever a new row is created, in accordance to the previous startingLevel value
...
// When a job is selected from a dropdown in the Job Details/Info section, it will populate with information from jobDetails.json
...
// for comparing initial and final values when comparison is struck.
...
function updatePreview() {
    ...
    if (comparisonMode === "custom") {
        ...
        //Visual display for custom path comparisons.
        document.getElementById("comparisonOutput").textContent =
    ...
    if (baselinePath === null) {
        document.getElementById("comparisonOutput").textContent =
            "There is nothing to compare. The final job must share a common job found earlier in its pathway."
        return;
    }
...
// Visual css component for html generated by Comparison.
...
// Hides display of comparison UI if mode not selected
    if (mode === "custom") {
